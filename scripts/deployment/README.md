...
This directory contains deployment scripts for the LazyVoter smart contract on the Hedera network.
...
| `--rpc-url` | string | Yes* | JSON-RPC endpoint (*required for Ethers path) | - |
...
3. **"rpc-url is required"**: Use `--rpc-url` for Ethers path or `--use-hts` for HTS path
4. **Gas estimation failures**: Try increasing `--gas-multiplier` or use HTS path
5. **Invalid timestamp**: Ensure start time is before end time and in the future
